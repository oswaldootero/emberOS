...
| `/crm/[id]` | Customer profile: header (name, badges, lifetime revenue, outstanding, invoices, avg order) + tabs: **Overview** (contact, sales info), **Sales** (invoice list), **Notes** (inline editable), **Activity** (audit timeline), **Analytics** (revenue trend, frequency, largest/avg order, first/latest purchase, CLV) |
