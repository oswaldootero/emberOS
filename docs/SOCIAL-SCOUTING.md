...
### Find accounts + daily hashtags (`/social/find`)

Needs only the OpenAI key already in place. Type who you're after in
plain English ("cigar lounges in Tampa", "whiskey & cigar reviewers
10K–100K") and the AI searches the web for matching public Instagram
accounts, returning handle, one-line summary, approximate followers when
a result shows them, and a source link. Each result has **Add influencer**
/ **Add prospect** and shows when the account is already tracked.

The top of the page is **Hashtags to watch today**: 10–12 hashtags with a
why, marked *post with* / *monitor* / both, plus a few accounts to watch.
It's generated by AI web research once a day (the `daily-jobs` cron at
11:00 UTC) or on first visit, and can be regenerated. Costs a few cents
per run.

Because this is a search engine's view of Instagram, it finds established
accounts well and brand-new or tiny accounts poorly. Always open the
...
type their **@handle** — that's the only required field. Optionally paste
the post link (Instagram → ⋯ → Copy link; note post links don't carry the
poster's name, profile links do and auto-fill the handle) and/or drop a
...
|---|---|---|
| **Find accounts** | `/social/find` | AI web search for Instagram accounts by description + daily hashtag brief. Needs only OpenAI. |
...
- **Photo/video tags** — polled from the `/tags` edge once a day by the
  `/api/cron/daily-jobs` Vercel cron (Vercel's Hobby plan allows daily crons only), or on demand with **Sync now**.
...
| AI scout (OpenAI web search): accounts + hashtag brief, parsers + tests | `src/server/social/scout.ts`, `scout-parse.ts`, `scout-parse.test.ts`, actions in `src/server/actions/scout.ts` |
...
| Webhook and cron routes | `src/app/api/webhooks/meta/route.ts`, `src/app/api/cron/daily-jobs/route.ts` |
