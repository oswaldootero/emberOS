...
import {
  createProspect,
  extractProspectFromScreenshots,
...
export function ProspectScanClient() {
  ...
  async function extract() {
    ...
    try {
      ...
      const r = await extractProspectFromScreenshots(fd);
      if (!r.ok) {
        toast.error(r.error);
        return;
      }
  ...
  async function create() {
    ...
    try {
      ...
      toast.success("Prospect created — run the AI analysis from its profile.");
  ...
  const rows: { label: string; value: string | null }[] = result
    ? [
        { label: "Business", value: result.fields.businessName },
        { label: "Type", value: result.fields.businessType },
        {
          label: "Address",
          value:
            [result.fields.street, result.fields.city, result.fields.state, result.fields.zipCode]
              .filter(Boolean)
              .join(", ") || null,
        },
        { label: "Phone", value: result.fields.phone },
        { label: "Email", value: result.fields.email },
        { label: "Website", value: result.fields.website },
        { label: "Instagram", value: result.fields.instagram },
        { label: "Facebook", value: result.fields.facebook },
        {
          label: "Google rating",
          value:
            result.fields.googleRating != null
              ? `★ ${result.fields.googleRating}${result.fields.reviewCount != null ? ` (${result.fields.reviewCount})` : ""}`
              : null,
        },
        { label: "Hours", value: result.fields.businessHours },
        { label: "Owner", value: result.fields.ownerName },
        { label: "About", value: result.fields.description },
    ...
    <div className="space-y-5">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Up to 3 images — an Instagram profile, a Google Maps listing, a
            website… anything that shows who they are.
          </CardDescription>
        </CardHeader>
          ...
          {previews.length > 0 && !result && (
            <Button variant="gold" onClick={extract} disabled={extracting} className="w-full">
              {extracting ? (
                ...
                <>
                  <ScanSearch className="h-4 w-4" /> Extract prospect info
                </>
              )}
            </Button>
          )}
        ...
        <Card className="border-ember-500/25">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {result.fields.businessName}
            </CardTitle>
            <CardDescription>
              Review what was found — everything stays editable on the prospect
              profile.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {result.existing && (
              <div className="rounded-md border border-amber-500/25 bg-amber-500/[0.06] p-3 text-xs text-ivory">
                Heads up —{" "}
                <Link
                  href={`/prospects/${result.existing.id}`}
                  className="text-ember-200 underline underline-offset-2"
                >
                  {result.existing.businessName}
                </Link>{" "}
                already exists in the pipeline. Creating again will make a duplicate.
              </div>
            )}
